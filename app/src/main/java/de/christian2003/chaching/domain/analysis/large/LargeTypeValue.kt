...
/**
 * Value object contains the analysis result of the value for a type.
 *
...
data class LargeTypeValue(
     ...
     */
    init {
        require(sum >= 0.0) { "Sum must not be negative" }
        require(avgPerTransfer >= 0.0) { "Average per transfer must not be negative" }
        require(avgPerNormalizedDate >= 0.0) { "Average per normalized date must not be negative" }
    }
